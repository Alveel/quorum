...
| `make release`          | Bump `appVersion`, commit, tag (`TYPE=patch\|minor\|major`) — then `git push && git push --tags` |
| `make release-chart`    | Bump chart `version` independently (chart-only changes) — then `git push`                        |
